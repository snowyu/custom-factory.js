...
class FlatFactory
  ...
  defineProperty FlatFactory, 'getRealNameFromAlias', (alias)->
    @__aliases[alias]
  defineProperty FlatFactory, 'formatName', (aName)->aName
  defineProperty FlatFactory, 'getFactoryItem', (aName, aOptions)->
    result = this[aName]
    ...
    result
  defineProperty FlatFactory, 'register', (aItem, aOptions)->
    ...
    this[vName] = aItem
    @
  defineProperty FlatFactory, 'unregister', (aName)->
    if isString aName
    ...
    !!aName
  defineProperty FlatFactory, 'forEach', (cb)->
    if isFunction cb
      for k in getObjectKeys @
        v = this[k]
        break if cb(v, k) == 'brk'
    @
  defineProperty FlatFactory, 'aliases', (aItem, aAliases...)->
  ...
  #   result = this[aName] if @hasOwnProperty(aName)
  #   if result is undefined and @__aliases
  #     # Is it an alias?
  #     aName = this.getRealNameFromAlias aName
  #     result = this[aName] if aName && @hasOwnProperty(aName)
  #   result
  ...
  #           vName = vName.substring(0, len - vLen)
  #     unless vName
  #       throw new TypeError('The registered item should has a name.')
  #   vName = @formatName vName
  #   result = @getFactoryItem(vName)
  ...
  constructor: (aName, aOptions)->
    ...
    if not (this instanceof FlatFactory)
      ...
      try vFactory = arguments.callee.caller
